...
def _client(tmp_path):
    ...
    app = create_app(workspace=SemanticAnnotationWorkspace(tmp_path), static_dir=None)
    return TestClient(app)


def _complete_overlay():
    label = SEMANTIC_LABEL_CLASSES[1]
...
def test_api_lists_six_classes_datasets_and_scenes(tmp_path):
    client = _client(tmp_path)
    classes = client.get("/api/label-classes")
    assert classes.status_code == 200
    payload = classes.json()
    assert [item["id"] for item in payload["classes"]] == list(range(6))
    assert payload["walkway"]["separate_layer"] is True
    scenes = client.get("/api/scenes?dataset=real_v2")
    assert scenes.status_code == 200
    assert scenes.json()["scenes"][0]["scene_id"] == "real-venue-0001"


def test_api_validates_completes_and_exposes_walkway_overlay(tmp_path):
    client = _client(tmp_path)
    ...
    walkway = client.get("/api/scenes/real_v2/real-venue-0001/walkway-overlay")
    assert walkway.status_code == 200
    assert walkway.headers["content-type"].startswith("image/png")
