...
def run_tests():
    ...
    print("\n[PASS] Test 1: Index loaded successfully with", f"{len(recommender.metadata_df):,} recipes.")
    ...
    target_id = 38 # Low-Fat Berry Blue Frozen Dessert
    t0 = time.time()
    ...
    # Test 5: Latency Benchmark (< 500ms across 520k recipes is stellar)
    assert latency_ms < 500.0, f"Ingredient matching latency must be < 500ms (got {latency_ms:.1f}ms)"
    assert latency_sim_ms < 500.0, f"Similar recipes latency must be < 500ms (got {latency_sim_ms:.1f}ms)"
    print(f"[PASS] Test 5: Latency benchmark PASSED (Ingredient lookup: {latency_ms:.1f}ms, Cosine similarity: {latency_sim_ms:.1f}ms).")
